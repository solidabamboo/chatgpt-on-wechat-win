        ...
        with open(base_dir, "r", encoding="utf-8") as f:
            ...
            if config["plugins"]["bridge_room"]["enabled"] == False:
                ...
                try:
                    ...
                    print(f"[WX] error occurred in handle_group: {e}")
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if reply.type == ReplyType.TEXT or reply.type == ReplyType.TEXT_:
            ...
            if context.get("isgroup", False):
                wait_seconds = random.uniform(conf().get("group_chat_reply_wait_min", 0), conf().get("group_chat_reply_wait_max", 0))
                logger.debug(f"[WX] delay {wait_seconds} seconds to process the next message ...")
